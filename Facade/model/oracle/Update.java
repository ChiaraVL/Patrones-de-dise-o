...
public class Update {
    ...
    public static String order(Order ent) {
        return "UPDATE ORDERS SET client_id = '" + ent.getClient() + "', order_date = TO_DATE('" + ent.getOrderDate() + "', 'YYYY-MM-DD'), status = '" + ent.getEstatus() + "', total = " + ent.getTotal() + " WHERE id = '" + ent.getId() + "';";
    }
